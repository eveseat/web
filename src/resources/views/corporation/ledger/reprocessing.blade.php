...
@section('ledger_content')
      ...
      @foreach ($periods->chunk(12) as $chunk)
        <ul class="nav justify-content-between">
          ...
          @foreach ($chunk as $period)
            <li class="nav-item">
              <a href="{{ route('corporation.view.ledger.reprocessing', ['corporation_id' => $corporation_id, 'year' => $period->year, 'month' => $period->month]) }}" class="nav-link">
  ...
  <div class="card">
    <div class="card-header">
      <h3 class="card-title">{{ trans_choice('web::seat.reprocessing', 2) }}
