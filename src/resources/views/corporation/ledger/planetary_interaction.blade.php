@extends('web::corporation.ledger.layouts.view', ['sub_viewname' => 'planetary_interaction', 'breadcrumb' => trans_choice('web::seat.pi', 2)])

@section('page_description', trans_choice('web::seat.corporation', 1) . ' ' . trans_choice('web::seat.pi', 2))
          ...
          @foreach ($chunk as $period)
            <li class="nav-item">
              @if(date('Y', strtotime($period->year . '-01-01')) == (request()->route()->parameter('year') ?: carbon()->isoFormat('YYYY')) && date('m', strtotime($period->year . '-' . $period->month . '-01')) == (request()->route()->parameter('month') ?: carbon()->isoFormat('MM')))
                <a href="{{ route('seatcore::corporation.view.ledger.planetary_interaction', [
                  $corporation,
