@extends('web::layouts.corporation', ['viewname' => 'industry_facility', 'breadcrumb' => trans('web::seat.industry_facility')])

@section('page_description', trans_choice('web::seat.corporation', 1) . ' ' . trans('web::seat.industry_facility'))
...
@section('corporation_content')
                    ...
                    @foreach ($chunk as $period)
                        <li class="nav-item">
                            @if(date('Y', strtotime($period->year . '-01-01')) == (request()->route()->parameter('year') ?: carbon()->isoFormat('YYYY')) && date('m', strtotime($period->year . '-' . $period->month . '-01')) == (request()->route()->parameter('month') ?: carbon()->isoFormat('MM')))
                                <a href="{{ route('seatcore::corporation.view.ledger.industry_facility', [
                  $corporation,
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center">
            <div class="col-auto me-5">
                <h3 class="card-title">{{ trans_choice('web::seat.industry_facility', 2) }} - {{ date("M Y", strtotime(sprintf('%d-%d-01', $year, $month))) }}</h3>
            </div>
        </div>
