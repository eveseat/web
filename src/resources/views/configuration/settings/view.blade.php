                ...
                <div class="box-body">
                    ...
                    <div class="form-group row">
                        <label class="col-md-4 col-form-label"
                            for="admin_contact">{{ trans('web::seat.admin_email') }}</label>
                        <div class="col-md-6">
                    ...
                    <div class="form-group row">
                        <label class="col-md-4 col-form-label"
                            for="tracking_id">{{ trans('web::seat.tracking_id') }}</label>
                        <div class="col-md-6">
                        ...
                        <div class="form-row align-items-center my-3" id="customlink{{ $loop->index }}">
                            ...
                            <div class="col-md-2 form-inline">
                                <select class="form-control" name="customlink-newtab[]">
                                    <option value="1" {{ $node->new_tab === false ? '' : 'selected="selected"' }}>Yes
                                    </option>
                                    <option value="0" {{ $node->new_tab === false ? 'selected="selected"' : '' }}>No
                                    </option>
                                </select>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <dl>
                ...
                <dt>
                    <i id="live-sde-status" class="fas fa-question-circle text-orange version-check" data-vendor="ccp"
                        data-name="eve_online_sde" data-version="{{ setting('installed_sde', true) }}"
                        data-bs-toggle="tooltip" title="Checking package status..."></i> Eve Online SDE
                </dt>
                <dd>
                    <ul>
    ...
    <script type="text/javascript">
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            fetch("{{ route('seatcore::check.sde') }}")
                ...
                .then(data => {
                    ...
                    if (live_sde != "error") {
                        ...
                        var liveSdeOutdated = (live_sde != liveSdeStatus.getAttribute('data-version'));
                        ...
                        liveSdeStatus.setAttribute('data-original-title', liveSdeOutdated ?
                            'At least one new version has been released !' : 'The package is up-to-date.');
                    }
                });
            ...
            copyVersions.addEventListener('click', function() {
                ...
                copyVersions.setAttribute('data-bs-original-title', 'Copied!');
                let btn_tooltip = bootstrap.Tooltip.getInstance(copyVersions);
                btn_tooltip.show();
            })

            copyVersions.addEventListener('mouseleave', function() {
                copyVersions.setAttribute('data-bs-original-title', 'Click to copy packages versions');
            })
        
            
            var tochecks = document.querySelectorAll('.version-check');
            for(i = 0; i < tochecks.length; ++i) {
                var toCheckPackage = tochecks[i];
                // fill the packages global variable
                installedPackages.vendor.push(toCheckPackage.getAttribute('data-vendor'));
                installedPackages.name.push(toCheckPackage.getAttribute('data-name'));
                installedPackages.version.push(toCheckPackage.getAttribute('data-version'));

                // skip third party checks
                if (toCheckPackage.getAttribute('data-vendor') == 'ccp')
                    return false;

                const checkReq = new FormData();
                checkReq.append('vendor', toCheckPackage.getAttribute('data-vendor'));
                checkReq.append('package', toCheckPackage.getAttribute('data-name'));
                checkReq.append('version', toCheckPackage.getAttribute('data-version'));

                await fetch('{{ route('seatcore::packages.check') }}', {
                    method: 'POST',
                    body: checkReq,
                    headers: {
                ...
                .then(data => {
                    ...
                    toCheckPackage.setAttribute('data-original-title', data.outdated ?
                        'At least one new version has been released !' :
                        'The package is up-to-date.');
            ...
            //         beforeSend: function() {
            //             changelogModal.find('.modal-content').append(
            ...
            //             changelogModal.find('.modal-body').html(xhr.statusText + xhr
            //                 .responseText);
            //         }
            //     });
            // });
