    ...
    <div class="col-md-4">
      <div class="panel panel-default">
        <div class="panel-heading">
          <h3 class="panel-title">{{ trans('web::seat.global') }}</h3>
        </div>
        <div class="panel-body">
          <form role="form" method="post" id="role_title_form">
            {{ csrf_field() }}
            <input type="hidden" name="role_id" value="{{ $role->id }}" />

            <div class="form-group">
              <label for="role_title">{{ trans('web::seat.role_name') }}</label>
              <input type="text" value="{{ $role->title }}" name="title" id="role_title" class="form-control" />
            </div>
          </form>
        </div>
        <div class="panel-footer clearfix">
          <input type="submit" form="role_title_form" class="btn btn-warning pull-right" />
        </div>
      </div>
    </div>
  </div>
